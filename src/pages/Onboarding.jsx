...
function StepBranding({ onNext }) {
  ...
  const valid = /^[a-z0-9-]{3,40}$/.test(subdomain);
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2">
          ...
          {!valid && subdomain && <div className="text-xs text-red-600 mt-1">Use lowercase letters, numbers, and dashes only.</div>}
        </div>
        <div className="md:col-span-2">
      ...
      <div className="mt-6 text-right">
        <button disabled={!valid} className={`px-4 py-2 rounded-lg text-white ${valid ? 'bg-blue-600' : 'bg-blue-300 cursor-not-allowed'}`} onClick={() => onNext({ brandColor, logoUrl, subdomain, customDomain })}>Continue</button>
      </div>
    </div>
  );
}
